...
static mp_obj_t wifi_pro_set_tx_power(mp_obj_t level_obj) {
    ...
    if (err != ESP_OK) {
        mp_raise_msg_varg(&mp_type_OSError,
            MP_ERROR_TEXT("esp_wifi_set_max_tx_power failed: %d"), err);
    }
    return mp_const_none;
}
static MP_DEFINE_CONST_FUN_OBJ_1(wifi_pro_set_tx_power_obj,
                                 wifi_pro_set_tx_power);
...
static mp_obj_t wifi_pro_send_raw(mp_obj_t packet_obj) {
    ...
    if (err != ESP_OK) {
        mp_raise_msg_varg(&mp_type_OSError,
            MP_ERROR_TEXT("esp_wifi_80211_tx failed: %d"), err);
    }
    return mp_const_none;
}
...
static mp_obj_t wifi_pro_set_promiscuous(mp_obj_t state_obj) {
    ...
    esp_err_t err = esp_wifi_set_promiscuous(state);
    if (err != ESP_OK) {
        mp_raise_msg_varg(&mp_type_OSError,
            MP_ERROR_TEXT("esp_wifi_set_promiscuous failed: %d"), err);
    }
    return mp_const_none;
}
static MP_DEFINE_CONST_FUN_OBJ_1(wifi_pro_set_promiscuous_obj,
                                 wifi_pro_set_promiscuous);

static const mp_rom_map_elem_t wifi_pro_module_globals_table[] = {
    { MP_ROM_QSTR(MP_QSTR_set_tx_power),
      MP_ROM_PTR(&wifi_pro_set_tx_power_obj) },
    { MP_ROM_QSTR(MP_QSTR_send_raw),
      MP_ROM_PTR(&wifi_pro_send_raw_obj) },
    { MP_ROM_QSTR(MP_QSTR_set_promiscuous),
      MP_ROM_PTR(&wifi_pro_set_promiscuous_obj) },
};

static MP_DEFINE_CONST_DICT(wifi_pro_module_globals,
                            wifi_pro_module_globals_table);
